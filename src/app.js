  ...
  return (
    <div className="pt-8 px-24 lg   :px-80 w-100%">
      ...
      <form onSubmit={saveTodoHandler}>
        <div className="flex mt-4 justify-between">
          ...
          <div className="w-2/12 flex justify-center">
            ...
            <button
              ...
              type="button" onClick={cancelEditHandler}
            >
              Cancel
            </button>
            }
        ...
        {message && <div className="text-xs text-red-500 mt-1 ml-2">{message}</div>}
      </form>
        ...
        {todos.map(function (item) {
          return (
            <div key={item.id} className="flex justify-between mb-6">
              <input checked={item.done} className="mr-2" type="checkbox" onChange={doneTodoHandler.bind(this, item)}></input>
              <p onClick={doneTodoHandler.bind(this, item)} className="text-lg w-10/12">{item.done ? <strike>{item.activity}</strike> : item.activity }</p>
              <div className="w-2/12 flex justify-center">
                <button
                  onClick={function () {
